    ...
    <div>
      <section className="relative h-[40vh] min-h-[350px] flex items-end bg-foreground">
        <div className="container mx-auto px-6 pb-16">
          <p className="section-overline text-background/40 mb-2">What We Offer</p>
          <h1 className="text-4xl md:text-5xl font-bold text-background">Our Services</h1>
          ...
          {services.map((s, i) => (
            ...
            >
              <div style={{ direction: "ltr" }}>
                <img src={s.img} alt={s.title} className="rounded-xl shadow-stack w-full h-80 object-cover" />
              </div>
              <div style={{ direction: "ltr" }}>
                <span className="section-overline">Service {String(i + 1).padStart(2, "0")}</span>
                <h2 className="text-2xl md:text-3xl font-bold text-foreground mt-2">{s.title}</h2>
                <p className="text-muted-foreground mt-4 leading-relaxed prose-limit">{s.desc}</p>
                <a
                  href="mailto:nick@carpinspainting.org"
                  className="inline-flex items-center gap-2 mt-6 rounded-[8px] bg-foreground text-background px-6 py-3 font-semibold text-sm hover:shadow-lg transition-all active:scale-95"
                >
